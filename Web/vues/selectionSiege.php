...
$nbA             = isset($_GET['nb_adultes']) ? (int)$_GET['nb_adultes'] : 1;
$nbE             = isset($_GET['nb_enfants']) ? (int)$_GET['nb_enfants'] : 0;
$nbB             = isset($_GET['nb_bebes'])   ? (int)$_GET['nb_bebes']   : 0;
$totalPassengers = $nbA + $nbE + $nbB;

// Chargement du vol
$vol = VolDAO::chercher($vol_id);
$passengersRaw = $_POST['passengers'];

// Récupère les données postées, puis insère sans redirection automatique
if ($_SERVER['REQUEST_METHOD'] === 'POST') {

  // Insertion en base pour chaque passager
  foreach ($passengersRaw as $idx => $pData) {
    $passager = new Passager(
      $pData['prenom']            ?? '',
      $pData['deuxieme_prenom']   ?? '',
      $pData['nom']               ?? '',
      $pData['date_naissance']    ?? '',
      $pData['email']             ?? '',
      $pData['telephone']         ?? '',
    );
    PassagerDAO::inserer($passager);
    // Récupération de l'ID via le setter dans la DAO
...
<body>
  ...
  <main class="main-content">
    ...
    <div class="content-wrapper">
      ...
      <div class="selection-panel">
        ...
        <!-- Infos passager -->
        <?php if (is_array($passengersRaw) && !empty($passengersRaw)): ?>
          <?php foreach ($passengersRaw as $idx => $p): ?>
            <div class="passenger-info" data-index="<?= $idx ?>">
              <div class="passenger-details">
                <h4>Passager <?= $idx + 1 ?></h4>
                <p><?= htmlspecialchars($p['prenom'] . ' ' . $p['nom']) ?></p>
                <h4>Numéro de siège</h4>
                <p class="selected-seat">--</p>
                <h4>Classe</h4>
                <p class="selected-cabin">Économie</p>
              </div>
              <div class="action-buttons">
                <button class="btn btn-outline" id="saveBtn">Enregistrer</button>
                <button type="button" class="btn btn-primary" id="nextBtn" onclick="window.location.href='index.php?action=selectionSieges&vol_id=<?= $retour_id ?>&nb_adultes=<?= $nbA ?>&nb_enfants=<?= $nbE ?>&nb_bebes=<?= $nbB ?>'">
                  Vol suivant
                </button>
              </div>
            </div>
          <?php endforeach; ?>
        <?php else: ?>
          <p>Aucun passager à afficher. <a href="index.php?action=formulaireVoyageur&vol_id=<?= $vol_id ?>&nb_adultes=<?= $nbA ?>&nb_enfants=<?= $nbE ?>&nb_bebes=<?= $nbB ?>">Retour au formulaire</a>.</p>
        <?php endif; ?>
      </div>
    </div>
  </main>
  ...
  <script>
    ...
    // Affecte la sélection de siège au passager actif
    function assignSeatToPassenger(index, seat, cabin) {
      const panel = document.querySelector(`.passenger-info[data-index="${index}"]`);
      if (!panel) return;
      panel.querySelector('.selected-seat').textContent = seat;
      panel.querySelector('.selected-cabin').textContent = cabin;
    }
    ...
    document.querySelectorAll('.seat.available').forEach(seatEl => {
      seatEl.addEventListener('click', () => {
        const code = seatEl.dataset.seat;
        const cabin = seatEl.classList.contains('business-seat') ? 'Business' : 'Économie';
        // Ici, on récupère l'index du passager que l'on souhaite mettre à jour
        // Par défaut, on prend le premier non rempli
        let targetIndex = 0;
        document.querySelectorAll('.passenger-info').forEach(panel => {
          const idx = parseInt(panel.dataset.index, 10);
          const seatText = panel.querySelector('.selected-seat').textContent;
          if (seatText === '--' && idx > targetIndex) targetIndex = idx;
        });
        assignSeatToPassenger(targetIndex, code, cabin);
        // Mettre à jour le champ caché pour envoi POST
        const hiddenInput = document.getElementById('selectedSeatsInput');
        const seats = hiddenInput.value ? hiddenInput.value.split(',') : [];
        seats[targetIndex] = code;
